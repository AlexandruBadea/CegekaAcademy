...
namespace PetShelter.BusinessLayer.Tests
{
    public class RescuePetTests
    {
        ...
        public async Task GivenPetTypeIsValid_WhenRescuePet_ThenNoExceptionIsThrown(PetType petType)
        {
            // Arrange
            SetupHappyPath();
            _request.Type = petType;

            // Act
            Func<Task> rescuePetAction = async () => await _petServiceSut.RescuePet(_request);

            // Assert
            await rescuePetAction.Should().NotThrowAsync<ArgumentException>();
        }
